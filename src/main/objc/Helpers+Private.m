...
{    
    ...
    [configPathArray replaceObjectAtIndex:[configPathArray count] - 1  withObject:@"cassandra.conf"];
    NSString *configPath = [NSString pathWithComponents:configPathArray];
    NSString *processLogPath = [@"~/Library/Logs/cassandra.log" stringByExpandingTildeInPath];
...
+ (NSArray *)_pidListForProcesses
{
    ...
    for (int k = 0; k < processCount; k++) {
        ...
        if (fullName == nil) 
            fullName = [NSString stringWithFormat:@"%s",proc->kp_proc.p_comm];
        if ([@"cassandra" isEqualToString:fullName] == YES)
