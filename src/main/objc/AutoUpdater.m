...
- (void)checkForUpdate
{
    if (self._updateStep != AutoUpdateStepIdle)
        return;
    self.hasUpdated = NO;
    NSMutableURLRequest *request = [NSMutableURLRequest requestWithURL:[NSURL URLWithString:@"https://github.com/remysaissy/cassandra-macosx-prefspane/raw/master/download/LATEST_VERSION"] cachePolicy:NSURLRequestReloadIgnoringLocalCacheData timeoutInterval:6];
